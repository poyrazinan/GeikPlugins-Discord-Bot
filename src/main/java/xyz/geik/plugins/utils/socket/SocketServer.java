...
public class SocketServer extends Thread {
    ...
    public static int port = Main.socketPort;
                ...
                try {
                    if (in.readByte() == 1) {
                        ...
                        boolean success = UtilSocket.readString(in, false)
                                .equals(UtilSocket.hash(String.valueOf(random_code) + Main.socketPassword));
                        if (success) {
                            out.writeInt(1);
                            UtilSocket.createLog(Main.socketSuccesfullyLogin);
                        } else {
                            out.writeInt(0);
                            UtilSocket.createLog(Main.socketWrongPassword);
                            this.connect_status = false;
                        }
                    } else {
                        UtilSocket.createLog(Main.socketWrongData);
                        this.connect_status = false;
                    }
